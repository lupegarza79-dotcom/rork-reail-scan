// @ts-nocheck
// Supabase Edge Function: trustops-resolve-appeal
// POST /trustops-resolve-appeal - Resolve an appeal (service-role internal)
// GET  /trustops-resolve-appeal?health - Health check
...
const ENDPOINT = "trustops-resolve-appeal";
...
serve(async (req: Request) => {
  ...
  if (req.method !== "POST") {
    return new Response(JSON.stringify({
      ...
      message: "Use POST to resolve an appeal",
      endpoint: ENDPOINT,
  ...
  try {
    ...
    if (!action || !["accepted", "rejected", "closed"].includes(action)) {
      return new Response(JSON.stringify({
        ok: false,
        error_code: "invalid_input",
        message: "action must be one of: accepted, rejected, closed",
        endpoint: ENDPOINT,
      }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    console.log(`[${ENDPOINT}] Appeal resolved:`, appealId, action);
